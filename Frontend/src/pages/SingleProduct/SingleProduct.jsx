...
const SingleProduct = () => {
  ...
  const uploadCart = async () => {
    if (color === null) {
      ...
      if (result.payload.response.data.message != null) {
        toast.error("Please kindly login again.");
        navigate("/login");
      } else {
        setTimeout(() => {
            ...
            <div className="main-product-row-01">
              ...
              <div className="main-product-coloum-02">
                <div className="main-product-details">
                  ...
                  >
                    <p className="price-product">NGN {productState?.price.toLocaleString()}</p>
                    <div className="main-product-div-01">
                      <ReactStars
                          count={5}
                          size={24}
                          value={parseFloat(productState?.totalrating) || 0}
                          edit={false}
                          activeColor="#ffd700"
                      />
                      ...
                      {productState &&
                          productState?.ratings?.map((item, index) => (
                              <div key={index} className="review-div-03">
                                <h4 style={{marginTop: "2.3px"}}>{item.postedby.firstname} {item.postedby.lastname}</h4>
                                <ReactStars
                                    count={5}
                                    size={24}
                                    value={item?.star}
                                    ...
                                    activeColor="#ffd700"
                                />
                                <p key={`comment-${index}`}>{item?.comment}</p>
                              </div>
                          ))}
                    </div>
                  </div>
